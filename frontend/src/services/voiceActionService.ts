...
class VoiceActionService {
    ...
    initialize() {
        ...
        voiceAppCommandBus.registerVoiceFeedback(this.handleAppFeedback.bind(this));
    }
    ...
    async executeVoiceCommand(functionName: string, params: any = {}) {
        ...
        if (functionName === 'show_upload_form') {
            ...
            return await voiceAppCommandBus.executeAppCommand('triggerAnalysis', params);
            
        } else if (functionName === 'set_lead_time') {
            return await voiceAppCommandBus.executeAppCommand('updateLeadTime', { value: params.leadTime });
            
        } else if (functionName === 'set_payment_terms') {
            return await voiceAppCommandBus.executeAppCommand('updatePaymentTerms', { value: params.paymentTerms });
            
        } else if (functionName === 'set_delivery_location') {
            return await voiceAppCommandBus.executeAppCommand('updateDeliveryLocation', { value: params.location });
            
        } else if (functionName === 'show_bom_analysis') {
            return await voiceAppCommandBus.executeAppCommand('showBOMAnalysis', params);
            
        } else if (functionName === 'show_commercial_terms') {
            return await voiceAppCommandBus.executeAppCommand('showCommercialTerms', params);
            
        } else if (functionName === 'show_rfq_preview') {
            return await voiceAppCommandBus.executeAppCommand('showRFQPreview', params);
            
        } else if (functionName === 'show_system_info') {
            return await voiceAppCommandBus.executeAppCommand('showSystemInfo', params);
    ...
    handleAppFeedback(event: string, data: any, context: any) {
        console.log(`Voice Action Service: Received app feedback '${event}'`, data);
