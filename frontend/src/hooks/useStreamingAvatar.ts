...
import StreamingAvatar, { StreamingEvents, StartAvatarRequest, TaskType, TaskMode } from '@heygen/streaming-avatar';
...
export const useStreamingAvatar = () => {
  ...
  const initAvatar = useMemoizedFn(async (token: string) => {
    try {
      ...
      avatar.on(StreamingEvents.AVATAR_TALKING_MESSAGE, (event: any) => {
        ...
        console.log('Avatar talking message (ignored):', event.detail?.message);
      });
      ...
      avatar.on(StreamingEvents.USER_END_MESSAGE, async (event: any) => {
        if (event.detail?.message) {
          ...
          try {
            const response = await avatarService.processUserInput(userMessage);
  ...
  const startAvatar = useMemoizedFn(async (config: StartAvatarRequest = DEFAULT_AVATAR_CONFIG) => {
      ...
      setTimeout(() => {
        if (avatarRef.current) {
          avatarRef.current.speak({
            text: "Hi! I'm Robbie, your AI procurement assistant. I'm here to help you create smart RFQs and optimize your procurement process. How can I assist you today?",
            taskType: TaskType.TALK,
            taskMode: TaskMode.ASYNC
          });
        }
